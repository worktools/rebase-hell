...
fn main() {
    let pid = get_service_pid(APP_PORT);
    if pid.is_none() {
        println!("no pid found for service.");
        std::process::exit(1);
    }
    let pid = pid.unwrap();
    let git_path = find_git_path(std::env::current_dir().unwrap());
    if git_path.is_none() {
        println!("Missing .git/ folder, not a valid path!");
        std::process::exit(1);
    }
    ...
    let wd_file_path = match std::env::var("REBASE_HELL_CONFIG_PATH") {
        Ok(path) => path,
        Err(_) => "~/.config/rebase-hell-working-directory.text".to_string(),
    };
    ...
    // kill -13 PORT
    let kill_command = format!("kill -13 {}", pid);
